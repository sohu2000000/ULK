 ...
 */
/*
 * mmput函数递减内存描述符的mm_user字段。
 * 如果该字段变为0，这个函数就释放局部描述符表、线性区描述符及由内存描述符所引用的页表，并调用mmdrop
 */
void mmput(struct mm_struct *mm)
{
	if (atomic_dec_and_test(&mm->mm_users)) {
		...
		put_swap_token(mm); /*★*/
        /*
         * 把mm_count字段减去1，如果字段为0，就释放mm_struct结构
         */
		mmdrop(mm); /*★*/
	}
}
