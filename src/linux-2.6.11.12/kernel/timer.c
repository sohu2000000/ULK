...
/*
 * 根据定时器的到到期时间，将timer 移动到base 中合适的位置上
 */
static void internal_add_timer(tvec_base_t *base, struct timer_list *timer)
{
	...
	/*
	 * 根据到期时间差计算定时器将要插入的位置vec 
	 * 到期时间在2^8 tick 内，进入t1的vec
	 * 到期时间在 2^(8+6) 个tick 内，进入t2 的vec
	 * 到期时间在 2^(8+2*6) 个tick 内，进入t3 的vec
	 * 到期时间在 2^(8+3*6) 个tick 内，进入t4 的vec
	 * 到期时间在 2^32  个tick 内，进入t5 的vec
	 * 到期时间超过2^32 个tick 的, 修正到期时间为2^32次方后，加入t5的vec
	 * 已超过到期时间的，加入到t1 中
	 */
 ...
 */
/*
 * 如果动态定时器已经被插入链表中，则调用mod_timer 函数来更新
 * expires字段的值，这样也能将对象插入到合适的链表中
 */
 ...
 */
/*
 * 从链表中删除定时器，然后检查该定时器函数是否还在其他CPU
 * 上运行，如果是，就等待，直到定时器函数结束
 * 执行速度慢，因为需要考虑定时器会激活自己的情况
 */
...
/**
 * 从定时器链表中删除一个定时器
 * 如果开发者知道定时器函数从不重新激活定时器，使用本函数速度快
 */
...
static inline void __run_timers(tvec_base_t *base)
{
	...
	/**
	 * 开始一个while 循环，当base->timer_jiffies 大于jiffies 的值时终止
	...
	while (time_after_eq(jiffies, base->timer_jiffies)) {
		 ...
		 */
		/*
		 * base->timer_jiffies  代表最早到期时间，如果没有积压，一般等于当前时间jiffies
		 * 注意只是挪动当前时间点(base->timer_jiffies) 对应的vec 元素上的链表
		 * 其他的vec 元素等到相应位的(base->timer_jiffies) 到达时候再触发移动, 具体解释参见<深入理解内核架构>15.2小节
		 * index ==0 就是说明当前时间点对应的256 个tick 内有没有要处理的定时器
		 * index == 0 说明在未来 2^8  个tick内没有到期的定时器，那么需要上移tv2-> tv1
		 * cascade(tv2) == 0 说明在未来 2^14  个tick内没有到期的定时器，那么需要上移tv3-> tv2
		 * cascade(tv3) == 0 说明在未来 2^20  个tick内没有到期的定时器，那么需要上移tv4-> tv3
		 * cascade(tv4) == 0 说明在未来 2^26  个tick内没有到期的定时器，那么需要上移tv5-> tv4
		 */		
		if (!index &&
 ...
 */
/*
 * 调用函数update_wall_time_one_tick 次，每次都给xtime.tv_nsec 加上1000000000
 * 如果必要就更新xtime.tv_sec
 */
...
static inline void update_times(void)
{
	...
	/*
	 * 计算jiffies 与上一次时间wall_jiffies 的差值
	 */
...
/*
 * alarm 系统调用的服务例程，
 * 在指定的时间后像进程发送ITIMER_REAL 信号
 * 因为是利用ITIMER_REAL 类型的动态定时器实现
 * 所以不能和具有ITIMER_REAL 参数的setitimer() 系统调用共同使用
 */
...
asmlinkage long sys_nanosleep(struct timespec __user *rqtp, struct timespec __user *rmtp)
{
	...
	ret = 0;
	/**
	 * schedule_timeout返回一个剩余节拍数。可能是系统调用被信号打断了(expire 不为0)。
	 * 在此自动重启系统调用。
	 */
	if (expire) {
