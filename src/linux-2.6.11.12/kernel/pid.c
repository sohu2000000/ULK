...
/*
 * pidhash_shift用来存放表索引的长度(以位为单位的长度)
 */
...
/*
 * 由于循环使用PID编号，内核必须通过管理一个pidmap_array位图来表示当前已分配的PID号和闲置的PID号
 */
...
static fastcall int __detach_pid(task_t *task, enum pid_type type)
{
	...
	pid = &task->pids[type];
    /*
     * 通过hlist_unhashed来判断task的pid是否是hash桶上的头结点，头结点一定在hash桶中,擦会进入下面括号中的内容
     */
