...
static void recalc_task_prio(task_t *p, unsigned long long now)
{
	...
	if (likely(sleep_time > 0)) {
		 ...
		 */
		/**
		 * Èç¹û½ø³Ì²»ÊÇÄÚºËÏß³Ì(mm!=NULL)£¬²¢ÇÒ½ø³Ì²»ÊÇ´ÓTASK_UNINTERRUPTIBLE×´Ì¬±»»½ĞÑ£¬²¢ÇÒË¯ÃßÊ±¼ä³¬¹ı¸ø¶¨µÄ½»»¥Ë¯ÃßÊ±¼ä¼«ÏŞ
...
task_t * context_switch(runqueue_t *rq, task_t *prev, task_t *next)
{
	...
	/**
	 * Èç¹ûÉÏÒ»¸öÏß³ÌÊÇÄÚºËÏß³Ì»òÕıÔÚÍË³öµÄ½ø³Ì£¬¾Í°ÑprevÄÚ´æÃèÊö·ûµÄÖ¸Õë±£´æµ½ÔËĞĞ¶ÓÁĞµÄprev_mmÖĞ¡£
	 * ²¢Çå¿Õrq->prev_mm
	 */
	if (unlikely(!prev->mm)) {
		prev->active_mm = NULL;
 ...
 * ´¦ÀíITIMER_VIRTUAL ÀàĞÍµÄ¼ä¸ô¶¨Ê±Æ÷
 * ÓĞÊ±ÖÓÖĞ¶Ï³ÌĞòµ÷ÓÃupdate_ process_times£¬È»ºóÃ¿¸öTICK µ÷ÓÃaccount_it_virt
 * cputime ´«ÈëµÄ¾ÍÊÇ1£¬¼´Ã¿TICK
 */
static inline void account_it_virt(struct task_struct * p, cputime_t cputime)
{
	/*
	 * È¡µÃ¶¨Ê±Æ÷Ê±¼äµ½it_virt ÖĞ
	 */
	cputime_t it_virt = p->it_virt_value;

	/*
	 * it_virt Îª0 ±íÊ¾Ã»ÓĞÆô¶¯ITIMER_VIRTUAL ÀàĞÍ ¶¨Ê±Æ÷£¬²»×ö´¦Àí
	 */
	if (cputime_gt(it_virt, cputime_zero) &&
	    cputime_gt(cputime, cputime_zero)) {
	    /*
	        * ¶¨Ê±Æ÷³¬Ê±£¬·¢ËÍSIGVTALRM µ½ÓÃ»§½ø³Ì£¬²¢ÓÃit_virt_incr ÖØĞÂ¸üĞÂµ½¶¨Ê±Æ÷Ê±¼ä
	        */
		...
		/*
		 * Ã¿TICK Îª¼õÉÙit_virt ¼õÉÙcputime, ÕâÀïÊÇÃ¿TICK ¼õ1£¬²¢½«it_virt ÖØĞÂÉèÖÃµ½½ø³ÌÃèÊö·ûµÄit_virt_value ÖĞ
		 */
...
/**
 * ¼ì²é½ø³Ì¶¨Ê±Æ÷£¬Çë²Î¼ûsettimerºÍalarmÏµÍ³µ÷ÓÃ
 * ´¦ÀíITIMER_PROF ÀàĞÍµÄ¼ä¸ô¶¨Ê±Æ÷
 * ÓĞÊ±ÖÓÖĞ¶Ï³ÌĞòµ÷ÓÃupdate_ process_times£¬È»ºóÃ¿¸öTICK µ÷ÓÃaccount_it_prof
 * cputime ´«ÈëµÄ¾ÍÊÇ1£¬¼´Ã¿TICK 
 */
static void account_it_prof(struct task_struct *p, cputime_t cputime)
{
	/*
	 * È¡µÃ¶¨Ê±Æ÷Ê±¼äµ½it_prof ÖĞ
	 */
	cputime_t it_prof = p->it_prof_value;

	/*
	 * it_prof Îª0 ±íÊ¾Ã»ÓĞÆô¶¯ITIMER_PROF ÀàĞÍ ¶¨Ê±Æ÷£¬²»×ö´¦Àí
	 */
	if (cputime_gt(it_prof, cputime_zero) &&
	    cputime_gt(cputime, cputime_zero)) {
	    /*
	        * ¶¨Ê±Æ÷³¬Ê±£¬·¢ËÍSIGPROF µ½ÓÃ»§½ø³Ì£¬²¢ÓÃit_prof_incr ÖØĞÂ¸üĞÂµ½¶¨Ê±Æ÷Ê±¼ä
	        */	    
		...
		/*
		 * Ã¿TICK Îª¼õÉÙit_prof ¼õÉÙcputime, ÕâÀïÊÇÃ¿TICK ¼õ1£¬²¢½«it_prof ÖØĞÂÉèÖÃµ½½ø³ÌÃèÊö·ûµÄit_prof_value ÖĞ
		 */		
...
void account_user_time(struct task_struct *p, cputime_t cputime)
{
	...
	/*
	 * ¸üĞÂÒ»Ğ©ÄÚºËÍ³¼ÆÊı£¬ÕâĞ©Í³¼ÆÊı´æ·ÅÔÚÃ¿CPU ±äÁ¿µÄkstatÖĞ
	 */
...
switch_tasks:
	...
	/*
	 * Í¬Ê±¼ì²éÆäËû½ø³ÌÊÇ·ñÉèÖÃÁËµ±Ç°½ø³ÌµÄTIF_NEED_RESCHED±êÖ¾£¬
	 * Èç¹ûÊÇ£¬ÔòÕû¸öschedule()º¯ÊıÖØĞÂ¿ªÊ¼Ö´ĞĞ£¬·ñÔòº¯Êı½áÊø
	 */
...
need_resched:
	/*
	 * ÉèÖÃpreempt_count ÖĞµÄPREEMPT_ACTIVE ±êÖ¾
	 */
...
void fastcall complete(struct completion *x)
{
	...
	/*
	 * ÔÚ²¹³äµÈ´ı¶ÓÁĞµÄ×ÔĞıËøÉÏµ÷ÓÃspin_lock_irqsave
	 */
...
asmlinkage long sys_sched_getparam(pid_t pid, struct sched_param __user *param)
{
	...
	read_lock(&tasklist_lock);
	/*
	 * Ê¹ÓÃfind_process_by_pid ÕÒµ½pid ËùÖ¸¶¨µÄ½ø³Ì
	 */
	p = find_process_by_pid(pid);
	retval = -ESRCH;
	if (!p)
    ...
    /*
        * security_task_getscheduler ½øĞĞÈ¨ÏŞµÈ°²È«ĞÔ¼ì²é
        */
	retval = security_task_getscheduler(p);
	if (retval)
		goto out_unlock;
...
long sys_sched_rr_get_interval(pid_t pid, struct timespec __user *interval)
{
	...
	read_lock(&tasklist_lock);
	/*
	 * find_process_by_pid ÕÒµ½pidËù¶ÔÓ¦µÄ½ø³Ì
	 */
	p = find_process_by_pid(pid);
	if (!p)
		goto out_unlock;
    ...
    /*
        * ½«½ø³ÌµÄÊ±¼äÆ¬×ª»»ÎªÃëÊıºÍÄÉÃëÊı
        * Í¨³£FIFO ½ø³ÌµÄÊ±¼äÆ¬µÈÓÚ0
        */
	jiffies_to_timespec(p->policy & SCHED_FIFO ?
				0 : task_timeslice(p), &t);
	read_unlock(&tasklist_lock);
	/*
	 * copy_to_user ½«½á¹û¿½±´µ½ÓÃ»§¿Õ¼ä
	 */
