 ...
 * 函数读取本地CPU 的软中断掩码并执行每个设置位相关的可延迟函数，
 * 只是执行固定个数的软中断，如果还有就放在ksoftirqd 中执行
 */
asmlinkage void __do_softirq(void)
{
	 ...
	 * 另外，需要注意的是：local_irq_save是关本地CPU的中断，而local_bh_disable是增加抢占计数中的软中断计数。（in_interrupt会阻止它们两个）
...
void local_bh_enable(void)
{
	...
	/**
	 * 如果有必要，就调用一次。
	 * 也就是检查本地CPU的TIF_NEED_RESCHED 标志是否被设置，
	 * 如果是，说明进程切换请求是挂起的，
	 * 因此调用preempt_schedule（）函数
	 */
