...
/*
 * 初始化8259A相关中断线，也就是初始化irq_desc数组
 */
void __init init_ISA_irqs (void)
{
	...
	//初始化irq_desc[]数组
	for (i = 0; i < NR_IRQS; i++) {
		irq_desc[i].status = IRQ_DISABLED; //状态:禁用
		irq_desc[i].action = NULL; //初始化为NULL.表示无中断处理函数,系统初始化完成之后,可以调用request_irq()注册中断处理函数
		irq_desc[i].depth = 1; //depth值为1,表示当前IRQ线被禁用
