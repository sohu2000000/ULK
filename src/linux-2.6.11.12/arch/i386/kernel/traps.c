...
static void default_do_nmi(struct pt_regs * regs)
{
	...
	if (!(reason & 0xc0)) {
		 ...
		 */
		/*
		 * 当SMP 系统启动设置了nmi_watchdog 参数，且前面条件没有满足的时候，
		 * 就说明该NMI watchdog 的喂狗操作
		 */
		if (nmi_watchdog) {
...
fastcall void do_nmi(struct pt_regs * regs, long error_code)
{
	...
	/*
	 * 调用对应的NMI 注册的处理函数，
	 * 如果没有相应的处理函数, 就使用default_do_nmi 进行默认处理
	 */
...
/*
 * 设置系统的中断描述符，都是通过统一的接口_set_gate()
 * type、dpl、段选择符不同
 * 陷阱门与中断门被锁定在内核态(DPL为0),系统门可以从用户态进入.
