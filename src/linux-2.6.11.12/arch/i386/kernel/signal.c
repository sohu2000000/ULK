	      ...
	      struct old_sigaction __user *oact)
{
	 ...
	 * 把新的new_ka 表拷贝到current->sig->action的在sig-1位置的表项中
	 */
...
asmlinkage int sys_sigreturn(unsigned long __unused)
{
	...
	current->blocked = set;
	/*
	 *重新调用recalc_sigpending 计算并设置TIF_SIGPENDING
	 */
	recalc_sigpending();
	spin_unlock_irq(&current->sighand->siglock);
...
/**
 * 与sys_sigreturn 相似，只不过是从扩展帧中恢复硬件上下文后删除扩展帧
 */
...
/*
 * 同setup_frame 行为相同，但是把用户态堆栈存放在一个扩展帧rt_sigframe 中
 * 这个帧也包含了与信号相关的siginfo_t 表的内容。此外，该函数设置pretcode
 * 字段指向__kernel_rt_sigreturn 代码
 */
...
int fastcall do_signal(struct pt_regs *regs, sigset_t *oldset)
{
	 ...
	 * do_signal在处理了一个单独的信号后返回，直到下一次调用do_signal()时才考虑其他挂起信号。
	 */
	if (signr > 0) {
