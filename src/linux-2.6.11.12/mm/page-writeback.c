 ...
 * 一般是1/500秒，但是可以通过/proc/sys/vm/dirty_writeback_centisecs调整。
 */
...
{
    /*通过__read_page_state()从每CPU的page_state统计量获得wbs中值，即从统计数量获得应该wb的数量信息*/
 ...
 */
/*
 * 通过wbs得到回写数量的状态信息，通过pbackground和pdirty得到触发回写的页面数量阀值
 * pbackground 对应 /proc/sys/vm/dirty_background_ratio
 * pdirty 对应 /proc/sys/vm/dirty_ratio
 */
static void
 ...
 * 执行background_writeout 回调函数的pdflush内核线程是由如下两个条件的进程唤醒的
 *      1. 对页高速缓存中的内容进行的修改
 *		2. 引起脏页部分增加到超过某个脏背景页的阀值(通常为系统中所有页的10%,  通过修改文件/proc/sys/vm/dirty_background_ratio来调整这个值)。
 *		
...
static void background_writeout(unsigned long _min_pages)
{
	...
	for ( ; ; ) {
		...
		/*
		 * 从每CPU 变量page_state 中读当前页高速缓存中的页和脏页的数量
		 * 如果脏页所占的比例低于给定的阀值，而且已经有至少min_pages  页被刷新到磁盘
		 * 该函数就终止。这个阀值通常大约是总页数的40%, 可以通过写文件/proc/sys/vm/dirty_ratio来调整这个值
		 */
		 ...
		 * (MAX_WRITEBACK_PAGES - wbc.nr_to_write) 表示这次writeback_inodes 写了多少个页面
		 */
...
{
    /*0表示回写所有脏页，这里计算脏页的总数量并写入nr_pages中*/
	if (nr_pages == 0) {
		struct writeback_state wbs;
        /*获得回写状态(回写数量)信息*/
		get_writeback_state(&wbs);
...
void __init page_writeback_init(void)
{
	...
	/*
	 * 回写脏页的工作委托给了被定期唤醒的pdflush内核线程
	 * 初始化期间函数建立wb_timer动态定时器
	 */
