 ...
 check_next_cluster:
	...
	got_page:
		...
		if (offset == si->highest_bit)
			si->highest_bit--;
		/**
		 * 满了。(最后一个，下一次就进入找不到的处理流程)
		 */
		/*
		 * liufeng:  这里可以优化，其实可以增加一个标记为标记已满
		 * 就不用在下次申请的时候再次进行那么多次循环了
		 */
...
static int try_to_unuse(unsigned int type)
{
	 ...
	 */
	/*
	 * 该函数从init_mm内存描述符开始，访问所有内核线程和进程的地址空间。
	 * 这是一个相当号是的函数，通常是以开中断运行的。因此，与其他进程同步也是关键的
     */
	start_mm = &init_mm;
	 ...
	 */
	/**
	 * 扫描交换区的所有换出页。(swap_map数组)
	 */
	/*
	 * 如果计数器(swap_map[i]) 等于0 或者SWAP_MAP_BAD，则对下一个页槽进行处理
	 */
	while ((i = find_next_to_unuse(si, i)) != 0) {
		 ...
		 */
		/*
              * 对于以init_mm为头部的双向链表中的每个内存描述符，调用unuse_process。
              * 这个耗时的操作扫描拥有内存描述符的进程的所有页表项
              * 并用这个新页框的物理地址替换页表项中每个出现的换出标识符
              * 为了反映这种移动，还要把swap_map数组中的页槽计数器减1，
              * 并增加这个页框的引用计数。
		 */
		/*
		 * 使用shmem_unuse检查换出的页是否用于IPC共享内存资源，并适当地处理那种情况
		 */		 
		shmem = 0;
		swcount = *swap_map;
		if (swcount > 1) {
...
/**
 * 使指定的交换区无效。使得specialfile所指定的交换区无效
 */
asmlinkage long sys_swapoff(const char __user * specialfile)
{
	 ...
	 * 其实就是找到一个交换区，它的swap_file就是制定的文件specialfile，找不到就返回错误
	 */
	...
	/**
	 * 调用security_vm_enough_memory(cap_vm_enough_memory)，检查是否有足够的空闲页框把交换区上存放的所有页换入。
	 * 如果不够，交换区就不能禁用，然后师傅文件对象，返回错误码。这只是粗略的检查，但可使内核免于许多无用的磁盘操作
	 * 当执行这项检查时，cap_vm_enough_memory要考虑slab高速缓存分配且SLAB_RECLAIM_ACCOUNT标志置位的页框，这样的页的数量存放在slab_reclaim_pages变量中
	 */
	...
	/**
	 * 清除该标志SWP_WRITEOK后，PFRA将不会再向交换区换出更多的页。
	 */
	p->flags &= ~SWP_WRITEOK;
	swap_list_unlock();
...
out_dput:
    /*
     * 调用file_close关闭前面打开的specialfile文件
     */
...
asmlinkage long sys_swapon(const char __user * specialfile, int swap_flags)
{
	...
	/* 新交换区数组索引等于nr_swapfiles: 它检查保留给交换区索引的位数是否足够用于
	 * 编码索引，如果不够，则返回错误代码，如果足够，就将nr_swapfiles 的值加1
	 * 
	 * 注: 因为nr_swapfiles是使用的交换区的最后一个索引，所以nr_swapfiles之前的交换区都已经用了，所以type应该等于nr_swapfiles
	 */	
	/*
	...
	for (i = 0; i < nr_swapfiles; i++) {
		...
		if (i == type || !q->swap_file)
			continue;
        /*该交换区还已经映射到其他交换区，就说明错误的开启交换区(address_space对象也就是mapping已经被其他交换区使用了)*/
	...
	if (S_ISBLK(inode->i_mode)) {
		 ...
		 */
		 /*
		   * 把该文件所在块设备的描述符地址存入交换区描述符的bdev 字段 
		   */
	...
	case 2:
		...
		/**
		 * 根据交换区大小设置lowest_bit和highest_bit(lowest_bit在上面被设置为了1)
		 */
		...
		/**
		 * 通过访问第一个页槽中的info.last_page 和info.nr_badpages 
		 * 计算可用页槽个数, 并存放在交换区描述符的pages字段里面。
		 * 而且把交换区中的总页数赋给max字段
		 */
