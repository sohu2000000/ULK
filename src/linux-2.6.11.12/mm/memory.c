...
void clear_page_range(struct mmu_gather *tlb, unsigned long start, unsigned long end)
{
	...
	for (i = pgd_index(start); i <= pgd_index(end-1); i++) {
        /*
         * "&PGDIR_MASK",保证了next是对齐的，"+ PGDIR_SIZE"保证是上对齐的
         */
		next = (addr + PGDIR_SIZE) & PGDIR_MASK; 
        /*
         * next > end 处理end不是上边界的，保证最大是end
         */
		if (next > end || next <= addr)
			next = end;
 ...
 *
 * pmd相没有下级PT表, 就分配一个页面作为PT表
 * 返回address所在pte项的地址VA
 */
pte_t fastcall * pte_alloc_map(struct mm_struct *mm, pmd_t *pmd, unsigned long address)
{
	if (!pmd_present(*pmd)) {
		...
		new = pte_alloc_one(mm, address); /*分配新的页表*//*★*/
		 ...
		 */
		if (pmd_present(*pmd)) { /*将pmd项设置为present*/
			pte_free(new);
			goto out;
		}
		mm->nr_ptes++;
		...
		unsigned long addr)
{
	...
	if ((vm_flags & (VM_SHARED | VM_MAYWRITE)) == VM_MAYWRITE) {
		ptep_set_wrprotect(src_pte); /*★*/ /*变成WR*/
		pte = *src_pte;
	}
		...
		unsigned long addr, unsigned long end)
{
	...
	d = dst_pte = pte_alloc_map(dst_mm, dst_pmd, addr); /*注意，已有PT需要临时分配，PUD,PMD在初始化进程的时候已经建立好*/ /*★*/
	if (!dst_pte)
		return -ENOMEM;
		...
		unsigned long size, struct zap_details *details)
{
	...
	for (offset=0; offset < size; ptep++, offset += PAGE_SIZE) {
		...
		if (pte_present(pte)) {
            ...
            //如果页面号合法，则转换为相应的page，如果页面被保留（不可以断开映射），page置为NULL
			if (pfn_valid(pfn)) {
				page = pfn_to_page(pfn);
            ...
            //单处理器，在tlb_remove_page里判断page的引用计数，如果没有引用了，调用free_page_and_swap_cache将页面释放
		...
		if (unlikely(details))
			continue;
		if (!pte_file(pte)) /*这里page_present为非的时候，才会有pte_file，也就是说被swap了才会走到下面的分支*/
            /*★*/
 ...
 */
/*
 * unmap_vmas用来释放pte所映射的页面
 * mm:进程描述符 vma:要删除的起始vma 
 * start_addr：要删除的线性区的起始地址
 * end_addr：要删除的线性区的结束地址 
 * details：在调用的时候置为了NULL ^_^
 */
	...
	for ( ; vma && vma->vm_start < end_addr; vma = vma->vm_next) {
		...
		while (start != end) {
			...
			if (is_vm_hugetlb_page(vma)) {
                ...
                //断开从start到start + block之间的映射，并释放对应的物理页框
 ...
 */
/*如果VMA标记该地址可以写，则设置页表项为可写*/ 
static inline pte_t maybe_mkwrite(pte_t pte, struct vm_area_struct *vma)
{
    /*如果VMA标记该地址可以写，则设置页表项为可写*/
    /*如果调试程序试图往被跟踪进程只读线性区中的页中写数据。内核不会设置相关标志。这里的if会处理这种特殊情况。*/
	...
	unsigned long address, pte_t *page_table, pmd_t *pmd, pte_t pte)
{
	...
	unsigned long pfn = pte_pfn(pte); /*★*/ /*获得原有页面的pfn*/
	pte_t entry;
	...
	if (!TestSetPageLocked(old_page)) {
		 ...
		 */
		/*
		 * 当count == 1 且没有打开交换的时候，是COW已经复制了新的页面，原有页面只有一个任务在引用，所以只需要为该任务将页框标记为可写即可。或者是父进程/子进程已经死完了，就剩下哥们一个了
         */
	 ...
	 * 想一想测不准原理吧。
	 */
	if (old_page == ZERO_PAGE(address)) {/*如果旧页框是0页*/ /*为0页面，说明是前面读请求第一次访问页面时候分配的0页面*/
		/**
	 ...
	 */
	page_cache_release(new_page); /*减少安全性检查时加的*/
	page_cache_release(old_page); /*旧页已经不再被当前进程拥有了。*/
	spin_unlock(&mm->page_table_lock);
	return VM_FAULT_MINOR;
...
void swapin_readahead(swp_entry_t entry, unsigned long addr,struct vm_area_struct *vma)
{
	 ...
	 */
	num = valid_swaphandles(entry, &offset); /*确定换入的数量*//*★*/
	for (i = 0; i < num; offset++, i++) {
		/* Ok, do the async read-ahead now */
		new_page = read_swap_cache_async(swp_entry(swp_type(entry),
							   offset), vma, addr); /*换入页面*//*★*/
		if (!new_page)
			break;
 ...
 * address-引起异常的线性地址。
 * page_table-映射address的页表项的地址。(pte)
	...
	pte_t *page_table, pmd_t *pmd, pte_t orig_pte, int write_access)
{
	...
	if (!page) {/* 页不在高速缓存中 */
		 ...
		 */
 		swapin_readahead(entry, address, vma); /*★*/ /*换入页面*/
		/**
		 ...
		 */
 		page = read_swap_cache_async(entry, vma, address); /*★*/ /*换入页面*/
	...
	if (vm_swap_full())
        /*
         * 这时候如果仅被异常进程拥有的话，页面引用计数就是2，也就是仅有交换高速缓存和当前进程拥有
         * 如果是2的话，remove_exclusive_swap_page就会从交换高速缓冲中删除页面
         */
    ...
    /*
     * 如果引起缺页的访问是一个写访问，且造成缺页的进程是页的唯一拥有者，
     * 那么函数还需要设置Dirty和Read/Write标志以防止无用的写时复制错误
     */
	...
	unsigned long address, int write_access, pte_t *pte, pmd_t *pmd)
{
	...
	pgoff = pte_to_pgoff(*pte); /*★*/ /*读取offset*/

	pte_unmap(pte);
	spin_unlock(&mm->page_table_lock);

	err = vma->vm_ops->populate(vma, address & PAGE_MASK, PAGE_SIZE, vma->vm_page_prot, pgoff, 0); /*从文件中读取页面的内容*//*★*/
	if (err == -ENOMEM)
		return VM_FAULT_OOM;
	if (err)
		return VM_FAULT_SIGBUS;
	...
	int write_access, pte_t *pte, pmd_t *pmd)
{
	...
	/**
	 * 页存在，但是为只读,且需要写入操作。就分配一个新的页框，并把旧页框的数据复制到新页框。即COW
	 */
	if (write_access) {
		if (!pte_write(entry))
			/**
