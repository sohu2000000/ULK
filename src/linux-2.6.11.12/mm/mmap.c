...
static void remove_vm_struct(struct vm_area_struct *vma)
{
	...
	if (file)
		fput(file);
    /*如果线性区是匿名的，则把它从mm->anon_vma所指向的匿名链表中删除*/
	anon_vma_unlink(vma);
...
/**
 * 确定新叶子节点在与给定线性地址对应的红黑树中的位置。并返回前一个线性区的地址和要插入的叶子节点的父节点的地址 
 *
 * 返回值VMA是第一个end大于addr的线性区间，有可能包括addr 
 *
 * 如果不能找到含有vma的addr
 *      vma就是含有addr的VMA
 *      通过rb_link返回RB当前节VMA节点指针
 *
 * 如果不能找到含有vma的addr
 *      返回值VMA是第一个end大于addr的线性区间
 *      rb_link一定为NULL，表示没有addr对应的vma节点
 *      通过rb_parent返回RB父节点指针，因为rbtree总是插入叶子节点，所以这里rb_parent总是最后一个被遍历到的那个节点
 *      通过pprev上一个VMA的地址
 */
 ...
 * 经过分裂后，对vma进行调整
 * @vma:    待调整的vma
 * @start:  待调整的vma的新的起始地址
 * @end:    待调整的vma的新的结束地址
 * @pgoff:  待调整的vma的新的pgoff
 * @insert: 新分裂出来，准备进行插入的new vma
 */
...
int expand_stack(struct vm_area_struct *vma, unsigned long address)
{
	...
	if (address < vma->vm_start) {
		...
		size = vma->vm_end - address; /*计算向下的新的VMA的总长度*/
		grow = (vma->vm_start - address) >> PAGE_SHIFT; /*计算向下增长的长度(单位为页)*/
...
static void unmap_vma(struct mm_struct *mm, struct vm_area_struct *area)
{
    ...
    /*
     * 执行内存描述符的mm->unmap_area方法。
     * 根据进程线性区的不同类型，可以选择arch_unmap_area()或arch_unmap_area_topdown()中的一个来实现
     * 如果必要, 在两种情况下都要更新mm->free_area_cache字段
     */
	...
	struct vm_area_struct *prev, unsigned long end)
{
    ...
    /*记录前一个prev vma的next的地址，以便后面对其进行更新*/
	insertion_point = (prev ? &prev->vm_next : &mm->mmap);

    /*遍历查找位于umap区间内的vma，第一个不在umap区间的vma的vm_start一定大于end,因为前面已经split_vma分裂好了*/
	do {
        /*从红黑树上删除*/
		rb_erase(&vma->vm_rb, &mm->mm_rb);
		mm->map_count--;
    ...
    /*更新prev的next指向umap的vma后面的vma上，相当于将所有umap的vma下链*/
	*insertion_point = vma;
    /*完成下链动作(清楚最后的后继)，这会儿所有下链的vma组成了以vma为头的一个链表*/
