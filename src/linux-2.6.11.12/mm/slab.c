...
static struct array_cache *alloc_arraycache(int cpu, int entries, int batchcount)
{
    ...
    /*
     * 如果slab中没有分配出来，就直接通过kmalloc分配
     */
	if (!nc)
		nc = kmalloc(memsize, GFP_KERNEL);
    /*
     * 初始化array_cache管理结构
     * 不分配真正的对象，只分配管理结构，avail为0，通过cache_alloc_refill进行真正对象的填充
     */
	if (nc) {
		nc->avail = 0;
		nc->limit = entries;
...
static int do_tune_cpucache (kmem_cache_t* cachep, int limit, int batchcount, int shared)
{
    ...
    /*
     * 在New.new中为每CPU本地高速缓存分配array_cache本地高速缓存管理结构并初始化
     */
	for (i = 0; i < NR_CPUS; i++) {
		if (cpu_online(i)) {
    ...
    /*
     * 将new.new中的array_cache初始化到指定的cachep的array中，并记录原来的本地高速缓存描述符到new中
     */
	...
	/*
	 * 在多处理器系统中，小对象使用slab高速缓存同样包含一个附加的本地高速缓存，
	 * 他的地址被存放子高速缓存描述的list.shared字段中。
	 * 共享的本地高速缓存被所有CPU共享，他使得空闲对象从一个本地高速缓存移动到另一个本地高速缓存的任务更加容易，
	 * 他的大小灯油batchcount的8倍
     */ 
...
static void enable_cpucache (kmem_cache_t *cachep)
{
	...
	/*
	 * 设置每个CPU本地高速缓存的大小，
	 * 这个大小取决于存放在slab高速缓存中对象的大小，
	 * 范围从1(相对非常大的对象)到120(相对于小对象)
     */
    ...
    /*
     * batchcount字段的初始值，
     * 也就是从一个本地高速缓存的块里添加或删除对象的个数，
     * 被初始化为本地告诉缓存大小的一半(第三个参数)
     */
