...
vma_address(struct page *page, struct vm_area_struct *vma)
{
    ...
    /*
	 * 如果目标地址是匿名页，则检查页的线性地址是否在线性区内。如果不是，则结束并返回-EFAULT
	 * (anon_vma可能存有不包含目标页的线性区)    
     */
	...
	struct vm_area_struct *vma, unsigned int *mapcount, int ignore_token)
{
    ...
    /*
     * 通过has_swap_token判断，如果该页属于一个线性区，该区域所在进程拥有交换标记，那么该函数认可这个交换标记并返回 1
     */
 ...
 * Liufeng: 该函数同时还会清理PG_referenced和accessed，本意是准备往inactive表移动时使用，而不是标记为PG_referenced，注意不要被名字误导了
 */
...
static int try_to_unmap_one(struct page *page, struct vm_area_struct *vma)
{
	 ...
	 * 如果目标地址是匿名页，则检查页的线性地址是否在线性区内。如果不是，则结束并返回SWAP_AGAIN
	 * (anon_vma可能存有不包含目标页的线性区)
	 */
	if (address == -EFAULT)
		goto out;
	...
	/**
	 * 检查指向目标页的页表项，满足以下条件的不进行回收释放。(主要是因为不能通过PTE真正的确定虚拟地址)
	...
	/**
	 * 验证线性区不是锁定或者保留的。
	 * 验证页表项中的访问标志(Accessed)位是否被清0。如果没有，则将它清0，并返回SWAP_FAIL，该标志位表示页在使用，因而不能被回收。
	 */
	 ...
	 * 如果指向页框的页表项与COW关联，而vma标识的匿名线性区仍然属于原页框的anon_vma链表。(page的count和mapcount不相等)
	 */
...
static int try_to_unmap_anon(struct page *page)
{
	...
	/**
	 * 获得anon_vma数据结构的自旋锁。页描述符的mapping字段指向anon_vma数据结构
	 */
	anon_vma = page_lock_anon_vma(page);
	/**
	...
	spin_unlock(&anon_vma->lock);
    /*
     * 返回最后调用try_to_unmap_one得到的值，SWAP_FAIL(失败)或SWAP_AGAIN(部分成功)
     */
	return ret;
}
...
static int try_to_unmap_file(struct page *page)
{
	...
	vma_prio_tree_foreach(vma, &iter, &mapping->i_mmap, pgoff, pgoff) {
		ret = try_to_unmap_one(page, vma);
		/**
		 * 如果页描述符的_mapcount字段表明引用该页框的所有页表项都已经找到，或者出现错误(SWAP_FAIL)，就结束搜索过程。
		 */
		if (ret == SWAP_FAIL || !page_mapped(page))
			goto out;
	}
	 ...
	 * 这可能是映射是非线性的，这样函数可能无法清0某些页表项.因为页描述符的index字段不再对应线性区中的页位置。
	 * 现在开始对文件非线性映射的线性区进行穷尽搜索。
	 *
	 * 双向链表以文件的所有非线性区间的描述符所在的page->mapping文件的address-space对象的i_mmap_nonlinear字段为根
	...
	/**
	 * 遍历非线性映射链表。计算cursor，max_nl_cursor，max_nl_size
	 * 因为搜索可能很费时，索引进行有线的扫描，而且通过试探去决定扫描线性区的哪一部分，
	 * vma->vm_private_data字段存放当前扫描的当前指针。
	 */
	 ...
	 * 因此，try_to_unmap_file()在某些情况下可能会找不到待停止映射的页。
	...
	/**
     * 双向链表以文件的所有非线性区间的描述符所在的page->mapping文件的address-space对象的i_mmap_nonlinear字段为根
     * 调用try_to_unmap_cluster，而try_to_unmap_cluster函数会扫描该线性地址所有对应的所有页表项，并尝试将他们请0
	 * 这里对非线性线性区进行有限扫描。
	 */
	do {
		list_for_each_entry(vma, &mapping->i_mmap_nonlinear,
