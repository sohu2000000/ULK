				...
				unsigned long min, unsigned long * flags)
{
    ...
    /*
     * 如果RA_FLAG_MISS置位，则预读窗的长度是当前窗长度减2，小于4时设置为4     
     * 否则，预读窗的长度是当前窗长度的4被或者2倍。
     */
		...
		struct list_head *pages, unsigned nr_pages)
{
    ...
    /*
     * 如果可能，通过使用多段bio描述符想通用块层发出读操作。这通过address_space对象专用的readpages方法实现。
     * 读到的页面插入了页高速缓存中，见ext3_readpages的add_to_page_cache
     */
    ...
    /*
     * 如果没有定义readpages方法，就通过反复调用readpage方法实现
     * 读到的页面插入了页高速缓存中
     */
	pagevec_init(&lru_pvec, 0);
			...
			unsigned long offset, unsigned long nr_to_read)
{
	...
	for (page_idx = 0; page_idx < nr_to_read; page_idx++) {
        ...
        /*将要读的页同页高速缓存进行比较，如果该页面已经在页告诉缓存内，即可跳过*/
		page = radix_tree_lookup(&mapping->page_tree, page_offset);/*★*/
		if (page)
			continue;

        /*
         * 从磁盘进行读之前，读请求所需要的全部页框是一次性分配的。如果不能一次性得到全部页框，
         * 预读操作就只在可以得到的页上进行。而且把预读推迟至所有页框都得到时再进行并没有多大意义
         */
				 ...
				 unsigned long actual)
{
	if (actual == 0) {
        ...
        /*
         * 最后所请求的全部256(VM_MAX_CACHE_HIT)页都已在高速缓存内
         * 复位当前窗和预读窗，将文件标记为在高速缓存内(RA_FLAG_INCACHE)
         */        
 ...
 */
/**
 * 每次page_cache_readahead创建一个新窗，他就开始对所包含页的读操作。为了读一大组页
 * page_cache_readahead调用blockable_page_cache_readahead。
 * 预读
 */
static int
			...
			struct file_ra_state *ra, int block)
{
	...
	if (block) { /*如果服务于块设备的请求队列是读阻塞的，就不进行读操作*/
		     ...
		     unsigned long req_size)
{
	 ...
	 */
	/*
	 * 没有禁止预读，与上次调用所访问的是同一个文件且只请求一页，退出
     */ 
	...
	if ((ra->size == 0 && offset == 0)	/* first io and start of file */
		...
		/* First sequential */
        /*
         * 设定当前窗，按照请求的页号数确定其起始请求页及长度。对当前窗进行读操作
         */
		 ...
		 */
		/*
		 * 请求页数大于当前窗的最大值，在当前窗后面建立一个新的预读窗，对预读窗进行IO操作
         */
		if (req_size >= max) {
	 ...
	 */
	/*
	 * 有部分请求页在当前窗，也就是说还有一部分在预读窗
     */
	if ((offset + newsize - 1) >= ra->ahead_start) {
        /*
         * 用预读窗取代当前窗口，再创建一个新的预读窗
         * 对预读窗进行IO操作
         */
...
/*
 * 
 * 函数通过将RA_FLAG_MISS置位和清除RA_FLAG_INCACHE标志来调整预读算法
 * 出现在do_generic_file_read()中调用了page_cache_readahead()进行了预读后，还是通过find_get_page()找不到缓存页的情况
 * 这就说明页框回收算法对预读的页框进行了回收，所以这时候就需要通过handle_ra_miss()来进行算法参数的调整了
 */
