...
/*
 * 释放一个PT页表对应的地址空间的页表([address,address+size] 范围内)
 */
				  ...
				  unsigned long size)
{
    ...
    /*
     * 遍历所有的PTE(该PT下的)，调用ptep_get_and_clear，清除页表项内容(包括PA、Present等)
     */
	do {
		pte_t page;
...
/*
 * 释放一个PMD表对应的地址空间的页表([address,address+size] 范围内)
 */
				  ...
				  unsigned long size)
{
    ...
    /*
     * 遍历所有的pmd项，调用unmap_area_pte清空页表
     */
	do {
...
/*
 * 释放一个PUD表对应的地址空间的页表([address,address+size] 范围内)
 */
			   ...
			   unsigned long size)
{
    ...
    /*
     * 遍历该范围的所有pud，调用unmap_area_pmd来清空页表
     */
	do {
...
void unmap_vm_area(struct vm_struct *area)
{
    ...
    /*
     * 遍历地址空间涉及的所有pgd项，调用unmap_area_pud清理页表
     */
...
found:
    /*
     * 删除vm_struct所对应的页表的内容
     */
	unmap_vm_area(tmp);
    /*
     * 将该vm_struct从vmlist表中摘下
     */
	*p = tmp->next;
	write_unlock(&vmlist_lock);
	return tmp;
