		...
		swp_entry_t entry, int gfp_mask)
{
	...
	if (!error) {
		...
		if (!error) {
            /*
             * 增加页面的引用计数，并设置PG_swapcache和PG_locked标志
             */
			page_cache_get(page);
			SetPageLocked(page);
			SetPageSwapCache(page);
			page->private = entry.val; /*★*/ /*设定了page的private为index(entry.val)，后面利用这个值就知道如何从SWAP中找到页面内容并读入了*/
 ...
 */
/**
 * 通过传递来的参数(换出页标识符)在交换高速缓存中查找页并返回页描述符的地址
 * 如果页不在交换高速缓存中，就返回0. 该函数调用radix_tree_lookup函数，
 * 把指向swapper_space.page_tree的指针和换出页标识符作为参数传递，以查找所需要的页
 *
			...
			struct vm_area_struct *vma, unsigned long addr)
{
	 ...
	 * 则释放分配的新页框。
     *
     * 因为前面是个while循环，在第一个循环中新分配页面new_page并将页面插入交换高速缓存，
     * 第二个循环中会从交换高速缓存中找到并用found_page记录，不再使用new_page，
     * 所以这里释放new_page的引用计数
	 */
	if (new_page)
