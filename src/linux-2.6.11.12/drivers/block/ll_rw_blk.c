 ...
 */
/**
 * 摘除一个请求队列。当"视线中"所有可合并的请求都被加入请求队列时，内核就显示调用该函数
 * 此外，如果请求队列中待处理的请求数超过了请求队列描述符的unplug_thresh字段中的值，那么IO调度程序也会去掉该请求队列
 */
 ...
 * 保持插入的时间超过了q->unplug_delay,激活动态定时器的时间用完，就会执行该函数
 */
static void blk_unplug_timeout(unsigned long data)
{
	...
	/**
	 * 唤醒内核线程kblockd所操作的工作队列kblockd_workqueue
...
get_rq:
	if (freereq) {/* 有可用描述符，使用可用描述符 */
		...
		if ((freereq = get_request(q, rw, GFP_ATOMIC)) == NULL) {
			...
			/**
			 * 不能直接分配一个请求，那么等待内存可用，并分配一个请求描述，反复分配直到成功为止
			 */
...
static inline void blk_partition_remap(struct bio *bio)
{
	...
	if (bdev != bdev->bd_contains) {
		...
		/**
		 * 更新计数值，根据读写方向，更新read_sectors、reads或write_sectors、writes值
		 */
		switch (bio->bi_rw) {
		case READ:
...
end_io:
		 ...
		 * 调用make_request_fn方法将BIO请求插入队列q中
		 */
...
void submit_bio(int rw, struct bio *bio)
{
	...
	/* 调用generic_make_request 执行真正的工作 */
 ...
 * 如果已经传送完请求中的所有数据块，那么end_that_request_first返回0，否则返回1，如果返回1，则中断处理程序重新调用策略例程，继续处理该请求
 * 否则，中断处理程序吧请求从请求队列中删除
 * rq: 请求队列描述符
