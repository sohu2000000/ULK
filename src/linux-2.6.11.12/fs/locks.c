		...
		unsigned int cmd)
{
	...
	fl->fl_file = filp;
	fl->fl_pid = current->tgid; /*注意，设置为tgid*/
	fl->fl_flags = FL_FLOCK;
	fl->fl_type = type;
	fl->fl_end = OFFSET_MAX; /*-1表示对整个文件加锁*/
...
static int flock_lock_file(struct file *filp, struct file_lock *new_fl)
{
	...
	for_each_lock(inode, before) {/* 遍历文件上的所有锁 */
		...
		if (new_fl->fl_type == fl->fl_type)
			goto out;
		/*
		 * 已经存在FL_FLOCK锁，但是类型不一样
		 */
		found = 1;
		/**
		 * 从索引节点锁链表和全局文件锁链表中删除已存在的FLOCK锁，唤醒等待的进程，并释放锁结构。
		 */
	...
	if (new_fl->fl_type == F_UNLCK)/* 如果进程执行开锁F_UNLCK，锁已经被释放，因此解锁操作可以直接退出了。 */
		return 0;
	 ...
	 */
	/*
	 * 如果进程找到同一个文件对象的FL_FLOCK锁，表明进程想要把一个已经拥有的读锁改变为一个写锁(反之亦然)
	 * 那么调用cond_resched给予其他更高优先级进程一个运行的机会
	 */
	if (found)/* 有FL_FLOCK锁存在，则给高优先级进程一个机会，使其可以先获得锁。 */
		cond_resched();

	lock_kernel();
	for_each_lock(inode, before) {/* 再次搜索链表，以验证现有的FL_FLOCK锁并不与申请的锁冲突。 */
		struct file_lock *fl = *before;
		if (IS_POSIX(fl))
			break;
		if (IS_LEASE(fl))
		 ...
		 * 检查是否有冲突锁
		 * 冲突的判定条件:
		 * 在索引节点的锁链表中，肯定没有FL_FLOCK写锁，此外，如果进程正在请求一个写锁
		 * 那么跟么就没有FLOCK锁
		 */
		...
		error = -EAGAIN;
		/* 
		 * 发现冲突锁，如果FL_SLEEP置位，表明允许阻塞，
		 * 将新锁(waiter)插入到blocker锁循环链表和全局阻塞链表中 
		 */
...
static int __posix_lock_file(struct inode *inode, struct file_lock *request)
{
	...
	if (request->fl_type != F_UNLCK) {/* 本次请求不是释放锁，则应当是申请锁 */
		for_each_lock(inode, before) {/* 遍历文件上的所有文件锁 */
			...
			if (!IS_POSIX(fl))/* 不是POSIX锁，忽略 */
				continue;
			/*
			 * 该锁是POSIX锁，检查这个锁是否与请求的锁互相冲突，
			 * 本质上，在索引节点的链表中，必定没有用于同一区的FL_POSIX写锁，并且，
			 * 如果进程正在请求一个写锁，那么同一个区字段也可能根本没有FL_POSIX锁。
			 * 但是，同一个进程所拥有的锁不会冲突，这就允许进程改变它已经拥有的锁的特性
			 * 不冲突跳过其他检查 
			 */
			...
			if (posix_locks_deadlock(request, fl))
				goto out;
			error = -EAGAIN;
			/* 不死锁，则将锁插(waiter)入到冲突锁(blocker)blocker链表和阻塞链表中 */
	...
	/*
	 * 只要索引节点的锁链表中不包含冲突的锁，就检查把文件区重叠起来的当前进程的所有FL_POSIX锁
	 * 当前进程想按需要对文件区中相邻的区字段进行锁定、组合和拆分。在重叠情况下，新锁总是代替旧锁
	 */
	error = 0;
...
int flock_lock_file_wait(struct file *filp, struct file_lock *fl)
{
	...
	for (;;) {
		/**
		 * 调用flock_lock_file，并检查返回值
		 * 无阻塞的获得文件锁
		 */
		error = flock_lock_file(filp, fl);
		/**
		 * 返回码为0(没有冲突现象)，则返回0(成功)
		 * 不能重试，则退出并返回错误码。
		 */
		if ((error != -EAGAIN) || !(fl->fl_flags & FL_SLEEP))
			break;
		 ...
		 */
		/*
		 * 不相容但能够进入睡眠，调用wait_event_interruptible，把当前进程插入到lock->fl_wait等待队列中并挂起他
		 * 当进程被唤醒，循环会首部执行
		 */
		error = wait_event_interruptible(fl->fl_wait, !fl->fl_next);
		if (!error)
		...
		/*
		 * 不相容的情况，且FL_SLEEP被清除，就释放file_lock描述符，并返回一个错误码-EAGAIN
		 */
		locks_delete_block(fl);
		break;
	}
...
asmlinkage long sys_flock(unsigned int fd, unsigned int cmd)
{
	...
	if (error)
		goto out_putf;
	if (can_sleep)/* 如果cmdLOCK_NB，不包含允许阻塞，设置FL_SLEEP标志 */
	...
	/**
	 * 如果定义了filp->f_op->flock方法，回调文件系统的flock函数。
	 */
...
int fcntl_setlk(struct file *filp, unsigned int cmd, struct flock __user *l)
{
	 ...
	 * and shared.
	 */
	if (IS_MANDLOCK(inode) &&/* 检查这个锁是否是一个强加锁 */
	    (inode->i_mode & (S_ISGID | S_IXGRP)) == S_ISGID &&
	    mapping_writably_mapped(filp->f_mapping)) {/* 文件是否有一个共享内存映射 */
		error = -EAGAIN;/* 这种情况下，说明文件正在被其他进程访问，不能申请强制锁。返回-EAGAIN */
		goto out;
	}

	/* 根据用户态传入的参数flock和inode的文件大小，初始化一个file_lock结构。 */
	error = flock_to_posix_lock(filp, file_lock, &flock);
	if (error)
		goto out;
	if (cmd == F_SETLKW) {/* 允许阻塞申请锁F_SETLKW，设置FL_SLEEP标志 */
	...
	switch (flock.l_type) {
	case F_RDLCK:
		if (!(filp->f_mode & FMODE_READ))/* F_RDLCK申请读锁，需要检查有读权限 */
			goto out;
		break;
	case F_WRLCK:
		if (!(filp->f_mode & FMODE_WRITE))/* F_WRLCK申请写锁，需要检查有写权限 */
			goto out;
		break;
	case F_UNLCK:
		break;
	...
	for (;;) {/* 多次尝试获取锁，因为可能会存在锁冲突 */
		error = __posix_lock_file(inode, file_lock);/* 调用__posix_lock_file不阻塞的获取文件锁 */
		/*
		 * 检查__posix_lock_file返回值
		 */
		/* 失败了，也不允许阻塞，则退出返回错误码 -EAGAIN*/
