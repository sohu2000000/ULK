...
struct file *filp_open(const char * filename, int flags, int mode)
{
	...
	namei_flags = flags;
	/*
	 * 用特殊的格式对访问模式标志O_RDONLY、O_WRONLY、O_RDWR进行编码
	 * 如果文件访问需要读特权，那么只设置namei_flags标志的下标为0的位
	 * 如果文件访问需要写特权，那么只设置namei_flags标志的下标为0的位
	 */
	/*
	 * 读写 0010 -> (0011&0011) -> 0011 读写都置位
	 * 写   0001 -> (0010 & 0011) -> 0010 写置位
	 * 读   0000 -> (0001 & 0011) -> 0001 写置位
	 */
	 ...
	 */
	/*
	 * 调用open_namei，校验、查找(创建)并返回文件相关的dentry和inode
	 */
	error = open_namei(filename, namei_flags, mode, &nd);
	if (!error)
		/*
		 * 调用dentry_open()函数，打开目录项对象对应的文件(分配file结构，并且与dentry和indoe关联)
	     */
...
struct file *dentry_open(struct dentry *dentry, struct vfsmount *mnt, int flags)
{
	 ...
	 * 检查打开文件的address_space对象是否有已经实现的direct_IO方法，如果没有则返回错误码
	 */
 ...
 *	flags:访问模式
 *	mode:创建文件需要的许可权限。
 *　如果成功，返回文件描述符，也就是指向文件对象的指针数组current->files->fd中分配给新文件的索引；否则，返回-1
 */
asmlinkage long sys_open(const char __user * filename, int flags, int mode)
{
	...
	if (!IS_ERR(tmp)) {
		/**
		 * 调用get_unused_fd获取一个未用文件描述符
		 */
		fd = get_unused_fd();
		if (fd >= 0) {
			/**
			 * 调用filp_open打开文件并返回file结构。
			 */
...
asmlinkage long sys_close(unsigned int fd)
{
	...
	if (fd >= files->max_fds)
		goto out_unlock;
	/**
	 * 获得存放current->files->fd[fd]中的文件对象的地址，如果为空，则返回错误码。
	 */
	filp = files->fd[fd];
	if (!filp)
		goto out_unlock;
	/*
	 * 把current->files->fd[fd] 置位NULL。
	 */
	files->fd[fd] = NULL;
	/**
