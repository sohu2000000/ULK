...
/*
 * 计算块设备的最后一块的索引值（存放在bdev->bd_inode->i_size中的块设备大小
 * 除以存放bdev->bd_block_size中的块大小得到该索引值，bdev指向块设备描述符）
 */
...
/**
 * 把相对于文件开始处的文件块号转换为相对于块设备开始处的逻辑块号，对于块设备文件来说，这两个值是一致的
		...
		struct buffer_head *bh, int create)
{
	 ...
	 * 如果超过，那么对于写操作(create)它返回-EIO，对于读操作返回0(超出块设备读也是不允许的，但是不返回错误代码)
 ...
 * 同普通文件的commit_write方法本质上做同样的事情，
 * 唯一不同的是，不检查写操作是否扩大了文件，你根本不可能在块设备文件的末尾追加字符来扩大它
 */
...
struct block_device *bdget(dev_t dev)
{
	...
	/*
	 * 在dev文件系统中找到设备的inode 
	 * 如果不存在这样的节点，那么就分配一个新的索引节点和新的块设备描述符
	 */
    ...
    /*
     * 如果是新分配的块设备描述符，就进行初始化并加入到all_bdevs中
	 */
...
static struct block_device *bd_acquire(struct inode *inode)
{
	...
	if (bdev) {
		 ...
		 * 将imapping字段设置为bedv索引节点中相应的字段的值，以便回收内存时候使用
		 */
		inode->i_bdev = bdev;
...
static int do_open(struct block_device *bdev, struct file *file)
{
	...
	if (!bdev->bd_openers) {/* 还没有打开 */
		...
		if (!part) {/* 是一个整盘，而不是分区 */
			...
			if (!bdev->bd_openers) {
				/*
				 * 从disk->queue请求队列中的hardsect_size字段中获得扇区的大小，使用该值适当的设置bdev->bd_block_size和bdev->bd_inode->i_blkbits
				 * 同时从disk->capacity中计算来的磁盘大小设置bedv->bd_inode->i_size字段
				 */
			...
			/**
			 * 再次调用bdget_disk。这次参数不一样，这次传递的是disk->first_minor次设备号
			 * 获得整盘的块描述符地址whole
			 */
			...
			if (ret)
				goto out_first;
			/**
			 * 设置描述符:dev是分区，则指向分区所在整盘描述符bdev->bd_contains 
			 * 否则指向块设备描述符
			 */
		...
		if (bdev->bd_contains == bdev) {
		    ...
		    /*增加bdev->bd_contains->bd_part_count引用计数*/
