...
{
    /*
     * 调用目录项对象的d_release()方法
     */
	if (dentry->d_op && dentry->d_op->d_release)
		dentry->d_op->d_release(dentry);

    /*
     * 调用call_rcu函数以注册一个会删除目录项对象的回调函数，该函数又会调用kmem_cache_free把对象是否给slab分配器
     */    
...
static inline void dentry_iput(struct dentry * dentry)
{
    ...
    /*
     * 从拥有者索引节点的目录项对象链表中删除
     * 调用d_iput目录方法或者iput()函数减少目录项的索引节点的引用计数器
     */
	if (inode) {
		dentry->d_inode = NULL;
 ...
 * 另外，目录项高速缓存起索引节点高速缓存控制器的作用，因此，当一个目录项对象被释放时，存放相应索引节点对象的页也变成未用状态，而最终被释放
 *		nr:			待回收的页框数。
 *		gfp_mask:	GFP掩码。
 */
static int shrink_dcache_memory(int nr, unsigned int gfp_mask)
{
	 ...
	 * 因此返回值实际就是未用目录项数，但是可以通过修改文件/proc/sys/vm/vfs_cache_pressure或通过相关的sysctl()系统调用，系统管理员可以改变这个变量值。
	 * 把值改为小于100，则使shrink_slab()从目录项高速缓存回收的页少于从LRU链表中回收的页。
	 * 反正，如把值改为大于100，则使shrink_slab()从目录项高速缓存回收的页面多于从LRU链表中回收的页面
	 */
...
/*
 * 查找parent目录下name目录所对应的dentry，
 */
// TODO: name指向文件也会在目录项缓存里面吗?
...
/**
 * 文件对象通过一个名为filp的slab高速缓存分配，filp描述符的地址存放在filp_cachep中
 * 用于分配file结构的slab分配器。
 */
