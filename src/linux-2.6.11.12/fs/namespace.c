...
static int do_umount(struct vfsmount *mnt, int flags)
{
	/**
	 * sb是超级块对象的地址。
	 * 从已经安装文件系统对象的mnt_sb字段检索超级块对象sb的地址
	 */
	struct super_block * sb = mnt->mnt_sb;
	int retval;
	...
	lock_kernel();
	/**
	 * 如果要求强制卸载，那么调用umount_begin，超级块操作中断任何正在进行的安装操作。
	 */
...
asmlinkage long sys_umount(char __user * name, int flags)
{
	 ...
	 * 如果查找的最终目录不是文件系统的安装点，则返回-EINVAL;
	 */
	if (nd.dentry != nd.mnt->mnt_root)
		goto dput_and_out;
	/**
	 * check_mnt检查：要卸载的文件系统还没有安装在命名空间中。请注意：有些特殊文件系统没有安装点。
	 * 如果要卸载的文件系统还没有安装在命名空间中，则返回-EINVAL;
	 */	
	if (!check_mnt(nd.mnt))
		goto dput_and_out;
			...
			int mnt_flags, char *name, void *data)
{
	 ...
	 * do_kern_mount 处理实际的安装操作，并返回一个新安装的文件系统描述符地址
	 */
		 ...
		 int mnt_flags, struct list_head *fslist)
{
	...
	/**
	 * 获得当前进程的写信号量，为写namespace作准备，因为要更改namespace结构
	 */
	down_write(&current->namespace->sem);
		  ...
		  unsigned long flags, void *data_page)
{
	...
	if (flags & MS_REMOUNT)
		...
		/**
		 * MS_BIND标志表示用户要求在系统目录树的另一个安装点上文件或目录能够可见。do_loopback函数来实现
		 */
		retval = do_loopback(&nd, dev_name, flags & MS_REC);
	else if (flags & MS_MOVE)
		/**
		 * MS_MOVE一般是表示用户改变已经安装文件系统的安装点。do_move_mount函数完成这个任务
		 */
		retval = do_move_mount(&nd, dev_name);
	else
		/**
		 * 一般的，用户是希望安装一个特殊文件系统或存放在磁盘分区中的普通文件系统。do_new_mount完成这个操作
		 */
		retval = do_new_mount(&nd, type_page, flags, mnt_flags,
				      dev_name, data_page);
dput_out:
	/*
	 * 调用path_release()终止安装点的路径名查找，并返回0
	 */
	path_release(&nd);
	return retval;
}
...
static void __init init_mount_tree(void)
{
	 ...
	 * 把字符串"rootfs"作为文件系统类型参数传递给它，并把该函数返回的新安装文件系统描述符的地址保存在mnt局部变量中
	 * do_kern_mount最终会调用rootfs的get_sb方法，即rootfs_get_sb。
	 */
