...
/**
 * 用于为特殊文件系统分配次设备号。
 * unnamed_dev_idr 包含指向一个辅助结构(记录当前在用的此设备号)的指针
 */
	...
	int (*fill_super)(struct super_block *, void *, int))
{
	...
	/**
	 * 调用sget()搜索文件系统的超级块对象链表type->fs_supers。
	...
	int (*fill_super)(struct super_block *, void *, int))
{
	 ...
	 * 由于rootfs是一种特殊的文件系统，没有磁盘超级块，因此只需要执行两个超级块操作
	 */
	error = fill_super(s, data, flags & MS_VERBOSE ? 1 : 0);
	if (error) {
		up_write(&s->s_umount);
...
/**
 * 文件系统安装操作的核心函数，检查文件系统类型标志以及确定安装操作是如何完成的;
...
do_kern_mount(const char *fstype, int flags, const char *name, void *data)
{
	 ...
	 *
	 * 由于每个特定与文件系统的方法使用不同的魔幻数，因此在试着安装根文件系统的时候,get_sb()的调用大都会失败
	...
	if (IS_ERR(sb))
 	...
 	if (error)
 		goto out_sb;
	/*
	 * 用新超级块对象的地址初始化mnt->mnt_sb
	 */
	mnt->mnt_sb = sb;
	/**
	 * 将mnt->mnt_root字段初始化为与文件系统根目录对应的目录项对象的地址。并增加其引用计数值。
	 */
	...
	mnt->mnt_parent = mnt;
	/*
	 * 用current->namespace的值初始化mnt->mnt_namespace
	 */
	mnt->mnt_namespace = current->namespace;
	/*
	 * 释放超级块对象的读/写信号量s_umount
	 */
	up_write(&sb->s_umount);
	put_filesystem(type);
	/*
