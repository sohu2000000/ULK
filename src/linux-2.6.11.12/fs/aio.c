...
static int aio_setup_ring(struct kioctx *ctx)
{
	...
	down_write(&ctx->mm->mmap_sem);
    /*
     * 调用do_mmap()为进程分配一个存放AIO环的新匿名线性区
     */
...
out:
    ...
    /*     
     * 因为一部IO操作仍在运行，aio_run_iocb会被再低调用，而这一次的调用者是aio_wq工作队列的内核线程
     * kiocb描述符跟踪IO数据传输的运行。
     */
	if (-EIOCBRETRY == ret) {
		/*
...
static int __aio_run_iocbs(struct kioctx *ctx)
{
	...
	while (!list_empty(&run_list)) {
        ...
        /*
         * 执行aiio_run_iocb()函数，它实际上调用ki_retry方法为相应的异步IO操作启动数据传输
         * 如果ki_retry方法返回-EIOCBRETRY，则表示异步IO操作已经提交但还没有完全成功；
         * 稍后在这个kiocb上，aio_run_iocb()函数会被再次调用；否则，调用aio_complete()，为异步IO操作在AIO环境的环中追加完成事件
         */        
		aio_run_iocb(iocb);
 ...
 */
/*
 * 系统调用，删除AIO环境，还删除含有对应AIO环的匿名线性区。
 * 这个系统调用阻塞当前进程直到所有正在运行的异步IO操作结束
 */
 ...
 */
/*
 * io_setup()系统调用为调用进程创建一个新的AIO环境。它有两个参数:
 * nr_events: 正在运行的异步IO操作的最大数目(这将决定AIO环的大小)
 * ctxp: 一个存放环境句柄的变量指针，这个句柄也是AIO环境的基础
 */
 ...
 * 最后aio_pread()返回从文件读入的有效字节数，或者，如果函数去顶请求的字节没有传输完，则返回-EIOCBRETRY
 * 
 */ 
static ssize_t aio_pread(struct kiocb *iocb)
{
    ...
    /*
     * 对于大部分文件系统，文件对象的aio_read()方法就是调用__generic_file_aio_read函数
     * 假如文件的O_DIRECT标志置位，函数就调用generic_file_direct_IO函数，但是在这种情况下，
     * __blockdev_direct_IO()函数不是阻塞当前进程使之等待IO数据传输完毕，而是立即返回。
     */
 ...
 * 最后aio_pwrite()返回写入的有效字节数，或者，如果函数去顶请求的字节没有传输完，则返回-EIOCBRETRY
 */
static ssize_t aio_pwrite(struct kiocb *iocb)
{
    ...
    /*
     * 对于大部分文件系统，文件对象的aio_write方法就是调用generic_file_aio_write_nolock
     * 假如文件的O_DIRECT表示置位，函数就调用generic_file_direct_IO函数
     */
...
ssize_t aio_setup_iocb(struct kiocb *kiocb)
{
    ...
    /*
     * 如果异步IO操作是一个读请求，那么对应kiocb描述符的ki_retry方法是由aio_pread()实现的
     */    
	case IOCB_CMD_PREAD:
		ret = -EBADF;
		...
		break;
    /*
     * 如果异步IO是一个写请求，那么对应的kiocb描述符的ki_retry方法是由aio_pwrite()实现的
     */
 ...
 */
/*
 * 为了开始异步IO操作，应用要调用io_submit()系统调用接口，该系统调用有3个参数
 * ctx_id:  由io_setup()(标识AIO环境)返回的句柄
 * iocbpp:  iocb类型描述符的指针数组的地址，其中描述符的每项描述一个异步IO操作
 * nr:      iocbpp指向的数组长度
 */ 
