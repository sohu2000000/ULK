...
ssize_t vfs_read(struct file *file, char __user *buf, size_t count, loff_t *pos)
{
	...
	/**
	 * 检查文件读权限，如果不允许，返回-EBADF
	 */
	if (!(file->f_mode & FMODE_READ))
		return -EBADF;
	/*
	 * 如果没有read或aio_read方法，返回-EINVAL
	 */
	if (!file->f_op || (!file->f_op->read && !file->f_op->aio_read))
		return -EINVAL;
	/**
	 * 用户态地址不可写。调用access_ok检查buf和count参数
	 */
	if (unlikely(!access_ok(VERIFY_WRITE, buf, count)))
		return -EFAULT;

	/**
	 * 对要访问的文件部分进行锁冲突检查。如果有冲突，就返回错误码，如果该文件已经被F_SETLKW命令请求，那么就挂起当前进程
	 */
	...
	if (!ret) {
		...
		if (!ret) {
			/**
			 * 调用文件系统的read或者aio_read，所有这些方法都返回实际传送的字节数
			 */
			if (file->f_op->read)
