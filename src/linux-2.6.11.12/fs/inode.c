...
static void dispose_list(struct list_head *head)
{
    ...
    /*
     * 通过调用clear_inode()和destroy_ionode()来删除索引节点对象
     */
	...
	for (nr_scanned = 0; nr_scanned < nr_to_scan; nr_scanned++) {
		...
		if (inode_has_buffers(inode) || inode->i_data.nrpages) {
			...
			if (remove_inode_buffers(inode))
                /*
                 * 要释放一个索引节点，必须释放与该节点关联的任何私有缓冲区，它使页高速缓存内(引用该索引节点的)不再使用干净页框无效
                 */
...
/*
 * 初始化inode的时候，当发现索引节点与设备文件相对应的时候，就调用init_special_inode，
 * 把索引节点对象的i_fop字段设置为def_chr_fops或def_blk_fops，
 * 正是这两个表的引入，才使得设备文件上所发出的任何系统调用都将激活设备驱动程序的函数而不是基本文件系统的函数
 * open调用dentry_open，后者分配一个新的文件对象并把其f_op字段设置为i_fops中存放的地址
 */
