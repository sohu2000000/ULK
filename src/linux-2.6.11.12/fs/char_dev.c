			   ...
			   int minorct, const char *name)
{
	...
	/**
	 * 分配一个新的char_device_struct结构并用0填充
	 */
	 ...
	 * 函数从末尾项开始继续向前寻找一个尚未使用的主设备号对应的空冲突链表，没有找到就返回错误
	 */
	if (major == 0) {
	...
	/*
	 * 将新的char_device_struct描述符插入冲突链
	 */
	cd->next = *cp;
	*cp = cd;
	write_unlock_irq(&chrdevs_lock);
	/*
	 * 返回新的char_device_struct地址
	 */
	return cd;
out:
	write_unlock_irq(&chrdevs_lock);
		    ...
		    struct file_operations *fops)
{
	...
	/**
	 * 调用__register_chrdev_region分配请求的设备号范围。
	 */
	cd = __register_chrdev_region(major, 0, 256, name);
	if (IS_ERR(cd))/* 设备号冲突，返回。 */
		return PTR_ERR(cd);

	/* 为设备驱动程序分配一个新的cdev结构，将kobject类型设置为ktype_cdev_dynamic*/
	cdev = cdev_alloc();
	if (!cdev)
		goto out2;
    ...
    /*
     * 关联设备号范围描述符char_device_struct和字符设备驱动描述符cdev
	 */
	cd->cdev = cdev;
 ...
 * 设备驱动程序模型为字符设备定义了一个kobject映射域，该映射域由一个kobj_map类型的描述符描述，并由全景变量cdev_map引用
...
/**
 * 动态分配cdev描述符，并初始化内嵌的kobject数据结构。在引用计数为0时候会自动释放该描述符
 */
