		...
		struct file *filp)
{
	...
	case F_SETFL:
		...
		break;
	/*
	 * F_GETLK:  确定由flock结构描述符描述的锁是否与另一个进程已获得的某个FL_POSIX锁互相冲突，在冲突的情况下，用现有的锁的有关信息重写flock结构
	 * F_SETLK:  设置由flock结构描述的锁。如果不能获得该锁，则这个系统调用返回一个错误码
	 * F_SETLKW: 设置由flock结构描述的锁。如果不能获得该锁，则这个系统调用阻塞，也就是说，调用的进程进入睡眠状态直到该锁可用为止
	 * 
	 * 对于加锁调用fcntl_getlk()或fcntl_setlk()系统调用
	 */
	case F_GETLK:
