...
#define RADIX_TREE_MAP_SIZE	(1UL << RADIX_TREE_MAP_SHIFT)   /*64 = 1<<6*/
#define RADIX_TREE_MAP_MASK	(RADIX_TREE_MAP_SIZE-1)         /*63*/
...
struct radix_tree_node {
	 ...
	 * tags[1]数组是写回标记。
	 *
	 * 注意这里是bit数组，也就是每个数组就是一个long类型的变量，所以使用的时候用tags[tag][0]就相当于得到了数组，
	 * 得到数组后，再通过offset设定对应的位，请参见tag_set()和tag_clear()函数
	 */
...
int radix_tree_preload(int gfp_mask)
{
	...
	preempt_disable();
	rtp = &__get_cpu_var(radix_tree_preloads); /*在radix_tree_node_alloc中从slab分配不出来节点的时候，使用预加载节点*/
			...
			unsigned long index, void *item)
{
	...
	if ((!index && !root->rnode) ||
		 ...
		 * 分配新节点是通过radix_tree_node_alloc函数实现的
		 */
...
void *radix_tree_lookup(struct radix_tree_root *root, unsigned long index)
{
	...
	while (height > 0) {
		...
		slot = (struct radix_tree_node **)
			((*slot)->slots +
				((index >> shift) & RADIX_TREE_MAP_MASK)); /*"& RADIX_TREE_MAP_MASK"用来屏蔽6位以上的部分，得到一个0~63的值*/
		shift -= RADIX_TREE_MAP_SHIFT;
		height--;
	}

	return *slot;  /*注意，返回的是slots[i]指向对象的指针，而不是slots的指针。例如，页缓存的话就是一个page的指针*/
}
			...
			unsigned long index, int tag)
{
	...
	while (height > 0) {
		...
		/*
		 * 注意pathp++ 的动作，是的指针下移，
		 * 这样下一循环pathp[0] 就相当于上一个节点,也就是本次循环里的pathp[1]
		 */
		pathp++;
		shift -= RADIX_TREE_MAP_SHIFT;
		height--;
	...
	do {
		...
		/**
		 * 调用tag_clear 清除当前结点的标记。
		 */
		tag_clear(pathp[0].node, tag, pathp[0].offset); /*★*/
		/**
	 	 * 然后检查节点数组中所有标记是否都被清0.
	 	 * 只有全部请0 了，才继续清除上一个节点的标志
	 	 */
		for (idx = 0; idx < RADIX_TREE_TAG_LONGS; idx++) {
...
void *radix_tree_delete(struct radix_tree_root *root, unsigned long index)
{
	 ...
	 * 该循环清除页面标记。
	 *
	 * 这部分代码解释参见radix_tree_tag_clear()函数中的注释
	 */
	do {
		int tag;
...
int radix_tree_tagged(struct radix_tree_root *root, int tag)
{
	 ...
	 * 只测试root的即可。
	 */
	// TODO: 为什么不测试 root->rnode->tags[tag] 是否不等于0 呢? 只要设置了一个位，就不会等于0了，这样不是更快吗?
	for (idx = 0; idx < RADIX_TREE_TAG_LONGS; idx++) {
		if (root->rnode->tags[tag][idx])
			return 1;
	}
