...
/**
 * 描述包含kobject对象的结构类型。本质上，它描述的是包括kobject的容器的类型
 */
struct kobj_type {
	/**
	 * kobject类型的release函数。
	...
	/**
	 * 当创建kobject时，赋予该对象的默认属性。sysfs文件系统的缺省属性列表
	 */
...
/*
 * 增加kset的引用计数，只需简单的调用内嵌kobject结构中的kobject_get
 */
...
/*
 * 减少kset的引用计数，只需简单的调用内嵌kobject结构中的kobject_put
 */
