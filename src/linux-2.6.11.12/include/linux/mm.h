...
/*
 * 交换高速缓存的所有页只是用一个swapper_space地址空间，因此只有一个基树
 * (由swapper_space.page_tree指向)对交换高速缓存中的页进行寻址。swapper_space
 * 地址空间的nrpages字段存放交换高速患侧中的页数
 */
 ...
 */
/*
 * PFRA处理的每个磁盘高速缓存在初始化的时候必须注册一个shrinker函数
 * shrinker函数有两个参数: 待回收的页和一组GFP分配标志。
 * 函数按照要求从磁盘高速缓存回收页，然后返回任然留在高速缓存的可回收页数
 */
...
/*
 * ULK:
 * In a two- or three-level paging system, this function does nothing: 
 * it simply returns the linear address of the Page Global Directory entry pgd
 * 
 * pgd相没有下级PUD表, 就分配一个页面作为PUD表
 * 返回address所在pud项的地址VA
 */
...
/*
 * ULK:
 * Defined so generic three-level paging systems can allocate a new Page Middle Directory for the linear address addr. 
 * If PAE is not enabled, the function simply returns the input parameter pud that is, the address of the entry in the Page Global Directory. 
 * If PAE is enabled, the function returns the linear address of the Page Middle Directory entry that maps the linear address addr. 
 * The argument cw is ignored.
 *
 * pud相没有下级PMD表, 就分配一个页面作为PMD表
 * 返回address所在pmd项的地址VA
 */
 ...
 * 该宏循环搜索所有线性区描述符，这些描述符在给定范围的线性地址中包含至少一个页
 */
