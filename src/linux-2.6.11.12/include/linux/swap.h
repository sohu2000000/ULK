...
/*
 * 如果页槽的计数器的值为SWAP_MAP_MAX(32767) ,那么存放这个页槽中的页就是"永久"的，并且不能从相应的页槽中删除
 */
#define SWAP_MAP_MAX	0x7fff
/*
 * 如果计数器的值为SWAP_MAP_BAD(32768)，那么就认为这个页槽是有缺陷的，也就是不可用的
 */
...
struct swap_info_struct {
	...
	/**
     * 存放在下一次分配时要检查的第一个页槽的索引
	 * 在搜索一个空闲页槽时要扫描的下一个页槽。
	 */
	unsigned int cluster_next;
	/**
     * 存放已分配的空闲页槽数,当函数从交换区的开头重新分配时该字段被重置为0
     * 存放的是空闲的页槽个数
...
/*
 * 如果该页属于一个线性区，该区域所在进程拥有交换标记，那么该函数认可这个交换标记并返回 1
 */
