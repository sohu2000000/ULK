...
/*
 * 每个类对象包括一个class_device描述符链表，每个描述符描述符了一个属于该类的单独逻辑设备
 * 同一个类中的设备驱动程序可以对用户态应用程序提供相同的功能
 * 类本质上是要提供一个标准的方法，从而为向用户态程序导出逻辑设备的接口
 */
struct class_device {
	...
	/*
	 * 指向一个设备描述符，一个逻辑设备总是对应于设备驱动程序模型中的一个给定设备，存在多个class_device描述符对应一个设备，
	 * 也就是一个硬件包括几个不同的子设备，每个子设备都需要一个不同的用户态接口
	 */
